...
const schema = {
  ...
  settings: {
    storage_method: "local", //Options are cloud, local and none
...
const loadDataCloud = async (user_id) => {
  const response = await fetch("./backend/index.php?user_id=" + user_id, {
    method: "GET",
    headers: {
      Accept: "application/json",
      "Content-Type": "application/json",
...
const mouseClickRuntime = (event) => {
  ...
  let elementTypeEditable =
    (element.localName.indexOf("text") > -1 ||
      element.localName.indexOf("input") > -1) &&
    element.contentEditable != "true"
      ? true
      : false;
  let elementTypeCheckbox =
    element.localName.indexOf("checkbox") > -1 ? true : false;
  let elementTypeRemove =
    element.localName.indexOf("remove") > -1 ? true : false;
...
const resetInputState = (event) => {
  ...
  let elementType =
    ...
    element.localName.indexOf("input") > -1
      ...
      : false;
  let resetText =
    element.dataset.type == "item-text" ||
    element.dataset.type == "subitem-text"
      ? true
      : false;
  if (elementType) {
...
const updateItemObj = (event, matrix, command) => {
  ...
  let text = "",
    ...
    complete = false,
    subItems = {};
  let parentItem = element.closest("item-wrapper")
    ? element.closest("item-wrapper").id
    : undefined;
  console.log("Command: " + command);
  switch (command) {
    case "edit_item":
      text = getInputValue(event);
      id = element.closest("item-wrapper")
        ? element.closest("item-wrapper").id
        : undefined;
    ...
    case "edit_subitem":
      text = getInputValue(event);
      id = element.closest("subitem")
        ? element.closest("subitem").id
        : undefined;
    ...
    case "add_subitem":
      ...
      document
        .querySelector(`[id="${parentItem}"] subitems`)
        .append(subItemNodeObject);
      break;
    case "add_item":
      text = getInputValue(event);
      id = generateId();
      ...
      break;
    case "toggle_item":
      container = element.closest("item-wrapper")
        ? element.closest("item-wrapper")
        : undefined;
      id = container.id;
      ...
      break;
    case "toggle_subitem":
      container = element.closest("subitem")
        ? element.closest("subitem")
        : undefined;
      id = container.id;
  ...
  Object.entries(items).forEach(([key, value]) => {
    ...
    if (item.subItems) {
      subItems = Object.entries(item.subItems).forEach(([key, value]) => {
        ...
        document
          .querySelector(`[id="${item.id}"] subitems`)
          .append(subItemNodeObject);
      });
    }
  });
